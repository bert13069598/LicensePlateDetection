...
for image_path in images:
    ...
    image_mask = np.zeros((imH,imW), np.uint8)
    ...
    # Get save cut filenames and paths
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            cv2.rectangle(image_mask, (xmin,ymin),(xmax,ymax),255,-1)

            image_crop=cv2.bitwise_and(image,image_crop,mask=image_mask)

            # save cut image
            crop_result_fn = base_fn +'_crop{}.png'.format(i)
            crop_savepath = os.path.join(CWD_PATH,RESULTS_DIR,crop_result_fn)
            cv2.imwrite(crop_savepath, image_crop)
